...
`ShowMore` inherits the opt-in `preserveMarkup` prop from `Truncate`. Enable it when the collapsed state needs to keep rendered inline markup such as links or inline styles.
...
Here is a basic example that could theoretically be suitable for any project. Adjust the controls to compare plain text, rich text, custom buttons, and preserveMarkup in different situations.
