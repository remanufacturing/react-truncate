...
`ShowMore` 会继承 `Truncate` 的 `preserveMarkup` 能力。如果折叠态需要保留链接、类名或内联样式等已渲染的内联富文本，可按需启用它。
...
这是一个理论上适用于任何项目的基本示例，可通过调节各项开关和滑块，对比纯文本、富文本、自定义按钮以及 preserveMarkup 在不同情况下的表现。
