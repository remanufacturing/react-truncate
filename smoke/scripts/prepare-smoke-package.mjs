...
const packageName = '@re-dev/react-truncate'
...
const installedPackagePath = path.resolve(
  consumerRoot,
  'node_modules/@re-dev/react-truncate',
)
const originalConsumerPackageJson = readFileSync(
...
try {
  execSync('pnpm install --force --no-lockfile --ignore-workspace', {
    cwd: consumerRoot,
    stdio: 'inherit',
  })
} finally {
